<template>
  ...
  const emit = defineEmits(['close', 'addAnimal', 'fetchBreeds']);
  ...
  const newOnFileChange = (e) => {
    if (e.target.files && e.target.files.length > 0) {
      const files = e.target.files;
  
      for (let i = 0; i < files.length; i++) {
        newAnimal.value.files.push(files[i]);
        const imageUrl = URL.createObjectURL(files[i]);
        newAnimal.value.images.push(imageUrl);
      }
    }
  };
  
  const fetchBreeds = (typeId) => {
    isTypeSelected.value = true;
    emit('fetchBreeds', typeId);
  };
  
  const addAnimal = () => {
    emit('addAnimal', newAnimal.value);
  };
  </script>